...
const FormField = <TFieldValues extends FieldValues = FieldValues, TName extends FieldPath<TFieldValues> = FieldPath<TFieldValues>>({ ...props }: ControllerProps<TFieldValues, TName>) => {
  const value = React.useMemo(() => ({ name: props.name }), [])
  return (
...
const FormLabel = React.forwardRef<React.ElementRef<typeof LabelPrimitive.Root>, ILabelProps>(({ className, extraLabel, ...props }, ref) => {
  const { error, formItemId } = useFormField()
  // @ts-ignore
  return <Label ref={ref} className={cn(error && 'text-text-error-primary', className)} htmlFor={formItemId} extraLabel={extraLabel} {...props} />
})
